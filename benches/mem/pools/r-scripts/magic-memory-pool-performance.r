...
data_file_clib_alloc = "results-bench-mem-pools-clib-allocator.csv"
data_file_clib_mempool = "results-bench-mem-pools-mempool-magic.csv"

result_dir = paste(dirname(rstudioapi::getActiveDocumentContext()$path), sep = "", "/../results");

data_file_path_clib_alloc   = paste(result_dir, sep = "/", data_file_clib_alloc)
data_file_path_clib_mempool = paste(result_dir, sep = "/", data_file_clib_mempool)

data_clib_alloc = read.csv(file=data_file_path_clib_alloc, head = TRUE, sep = ",")
data_clib_mempool = read.csv(file=data_file_path_clib_mempool, head = TRUE, sep = ",")

data_clib_alloc_agg = ddply(data_clib_alloc,~alpha,summarise,mean=mean(1000/call_duration_ms),sd=sd(1000/call_duration_ms))
...
plot(data_clib_alloc_agg$mean)
plot(data_clib_mempool_agg$mean)

df = data.frame(
    ops_per_sec = c(data_clib_alloc_agg$mean, data_clib_mempool_agg$mean),
    sd = c(data_clib_alloc_agg$sd, data_clib_mempool_agg$sd),
    ratio = c(data_clib_alloc_agg$alpha, data_clib_mempool_agg$alpha),
    label = c(rep("clib allocator", length(data_clib_alloc_agg$mean)), rep("mempool (improved)  ", length(data_clib_mempool_agg$mean)))
)
