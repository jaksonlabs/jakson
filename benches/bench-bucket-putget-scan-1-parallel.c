...
#define TYPE "SIMPLE_SCAN_1_PARALLEL"
        ...
        for (size_t sample = 0; sample < 1; sample++) {
            for (int i = 5; i <= 100; i += 5) {
                for (int rep = 0; rep <= 100; rep += 10) {
                    for (size_t nthreads = 0; nthreads < 64; nthreads += 4) {
                        ...
                        string_hashtable_create_scan1_parallel(&map, NULL, 1, CURRENT_BYTES, 1.7f, nthreads);
                        ...
                        printf("%s;%zu;%zu;%f;%f;%f;%f;%zu;%d;%zu\n",
                                TYPE,
                                CURRENT_BYTES,
                                sample,
                                i/100.0f,
