...
static int simple_create_extra(struct string_hashtable *self, float grow_factor, size_t num_buckets,
...
static int simple_map_insert(struct vector of_type(simple_bucket)* buckets, char* const* keys,
        const uint64_t* values, size_t* bucket_idxs, size_t num_pairs, struct allocator *alloc, size_t nthreads);
...
static int simple_bucket_insert(struct simple_bucket *bucket, const char *key, uint64_t value,
        struct allocator *alloc, size_t num_threads) force_inline;
...
static size_t simple_bucket_find_entry_by_key(struct simple_bucket *bucket, const char *key, struct allocator *alloc, size_t num_threads);
...
int string_hashtable_create_scan1_parallel(struct string_hashtable* map, const struct allocator* alloc, size_t num_buckets,
...
static int simple_map_fetch(struct vector of_type(simple_bucket) *buckets, uint64_t *values_out, bool *key_found_mask,
        ...
        struct allocator *alloc, size_t num_threads)
{
...
static int simple_map_remove(struct simple_extra *extra, size_t *bucket_idxs, char *const *keys, size_t num_keys, struct allocator *alloc, size_t num_threads)
{
    struct simple_bucket *data = (struct simple_bucket *) vector_data(&extra->buckets);
...
static int simple_bucket_insert(struct simple_bucket *bucket, const char *key, uint64_t value, struct allocator *alloc, size_t num_threads)
{
