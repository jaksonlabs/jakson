...
static int simple_create_extra(struct string_lookup *self, float grow_factor, size_t num_buckets,
...
static int simple_map_insert(struct vector of_type(simple_bucket)* buckets, char* const* keys,
        const string_id_t* values, size_t* bucket_idxs, size_t num_pairs, struct allocator *alloc, size_t nthreads);
...
static int simple_bucket_insert(struct simple_bucket *bucket, const char *key, string_id_t value, struct allocator *alloc, size_t num_threads)
{
